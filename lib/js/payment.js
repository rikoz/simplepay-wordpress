function formatAmount(amount) {
    ...
    var lower_amount = strAmountList[0];

    if (decimalPlaces === 0) {
        lower_amount += '00';
    } else if (decimalPlaces === 1) {
        lower_amount += strAmountList[1] + '0';
    } else if (decimalPlaces === 2) {
        lower_amount += strAmountList[1];
    } else if (decimalPlaces > 2) {
        lower_amount += strAmountList[1].substring(0, 2);
    }

    strAmountList[0].indexOf('0') === 0 ? lower_amount = String(parseInt(lower_amount)) : void 0;
            ...
            jQuery.ajax({
                ...
                success: function (response) {
                    if (response.result === 'failure' && !response.messages) {
                        ...
                        var WCEmail = jQuery('input[name="billing_email"]').val();
                        ...
                        var handler = SimplePay.configure({
                            ...
                            platform: 'Wordpress ' + checkout_page.simplepay_plugin_version,
                        ...
                        var paymentData = {
                            ...
                            city: WCCity,
                            country: WCCountry,
                            amount: formatAmount(simplepay_cart_total),
